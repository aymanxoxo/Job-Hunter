...
Produced by the AI engine from a profile, or edited by the user. Immutable (``frozen``).
``min_score_threshold`` defaults to ``DEFAULT_MIN_SCORE`` and is the single effective results filter
(ADR-006); config only seeds that default at generation time.
"""
